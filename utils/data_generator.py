...
def generate_dataset(
    ...
    for k in range(n_clusters):
        model = LSTMSinglePointProcess(
            n_classes + 1, hidden_size, num_layers, n_classes
        )
        data[
            k * n_points_per_cluster : (k + 1) * n_points_per_cluster, :, :
        ] = model.simulate(
            n_points_per_cluster,
            torch.rand(n_points_per_cluster) * dt_scaling_param,
            n_timestamps,
            True,
        )

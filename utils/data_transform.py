...
def transform_data(args):
    ...
    if ext == "pkl":
        ...
        info = {"classes": list(map(int, classes)), "seq_nmb": int(seq_nmb)}
        ...
        for file in sorted(
            ...
            if file.endswith(f".{ext}") and re.sub(fr".{ext}", "", file).isnumeric():
                ...
                if datetime:
                    ...
                    df[time_col] = (df[time_col] - df[time_col][0]) / np.timedelta64(
                        1, "D"
                    )
                df.to_csv(Path(save_dir, file.replace(ext, "csv")))
        seq_nmb = nb_files
        classes = list(classes)
        info = {"classes": classes, "seq_nmb": seq_nmb}
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        help="name of Dropbox file with dataset",
    )
    parser.add_argument(
        "--data_dir",
        type=str,
    ...
    parser.add_argument(
        "--maxlen", type=int, default=500, help="maximum length of sequence"
    )
    parser.add_argument(
        "--ext", type=str, default="tar.gz", help="extention of files with sequences"
    )
    parser.add_argument(
        "--datetime",
        type=bool,
        default=False,
        help="if time values in event sequences are represented in datetime format",
    )
    parser.add_argument(
        "--save_dir", type=str, default="./", help="path to save results"
    )
    parser.add_argument(
        "--maxsize", type=int, default=None, help="max number of sequences"
    )

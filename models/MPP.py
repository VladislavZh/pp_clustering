...
class MultiplePointProcesses(nn.Module):
    """
    ...
    def __init__(
        ...
        """
        input:
               model - str, type of the model: lstm, rnn or cnn
               input_size - int, input size of the data for the model
               hidden_size - int, model hidden state size
               ...
               batch_first - bool, whether the batch should go first in the model
               dropout - float (>=0,<1), dropout probability for all model layers but the last one
               bidirectional - bool, bidirectional (LSTM) or not

        model parameters:
               model - torch.nn.Module (lstm or rnn)
               modelname - str, type of model: lstm, rnn or cnn
        ...
        bidir_coef = 2 if bidirectional else 1
    ...
    def init_weight(self, *args):
        ...
        output:
               weights - torch.Tensor, size = args
        """
    ...
    def forward(self, s, return_states=False):
        ...
        for k in range(self.num_clusters):
            ...
            # model forward pass
            if self.modelname == "lstm":
                out, (hidden, cell) = self.model(s, (hidden0, cell0))
            elif self.modelname == "rnn":
                out, (hidden) = self.model(s, (hidden0))
                cell = cell0

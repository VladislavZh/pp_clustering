    ...
    def random_permutation(self):
        """
            ...
            output:
                   None
        """
        new_order = np.random.permutation(self.num_clusters)
        self.hidden0 = Parameter(self.hidden0[new_order, :, :])
        self.cell0 = Parameter(self.cell0[new_order, :, :])
...
class LSTMMultiplePointProcessesNoFinalLayer(nn.Module):
    """
        Multiple Point Processes Model without final layer
    """

    def __init__(self, input_size, hidden_size, num_layers, num_classes, n_steps,
                 batch_first=True, dropout=0, bidirectional=False):
        """
           input:
                  input_size - int, input size of the data for LSTM
           ...
           model parameters:
                  ...
                  lstm - torch.nn.Module, LSTM model
                  bn - torch.nn.Module, Batch Normalization
                  hidden0 - torch.nn.parameter.Parameter, initial hidden states, size[0] = 1
                  cell0 - torch.nn.parameter.Parameter, initial cell states, size[0] = 1
                  num_classes - int, number of types of events that can occur
                  bidir - bool, bidirectional LSTM or not
        """
        super().__init__()
        ...
        self.bn = nn.BatchNorm1d(n_steps)
        if bidirectional:
            self.hidden0 = Parameter(self.init_weigh(1, num_layers * 2, hidden_size))
            self.cell0 = Parameter(self.init_weigh(1, num_layers * 2, hidden_size))
        else:
            self.hidden0 = Parameter(self.init_weigh(1, num_layers, hidden_size))
            self.cell0 = Parameter(self.init_weigh(1, num_layers, hidden_size))
        self.num_classes = num_classes
        self.bidir = bidirectional
    ...
    def forward(self, s, return_states=False):
        """
            ...
            output:
                   out - torch.Tensor, size = (batch size, sequence length, hidden_size)
        """
        bs, seq_len, _ = s.shape
        ...
        if return_states:
            return out, hiddens, cells
        return out


class LSTMMultiplePointProcessesFinalLayer(nn.Module):
    """
        Multiple Point Processes Model, Point Processes are distinguished with different initial hidden states
    """

    def __init__(self, hidden_size, num_classes, num_clusters, n_steps,
    ...
    def forward(self, out, hidden0, cell0):
        """
            forward pass of the model

            input:
                   out - torch.Tensor, size = (batch size, sequence length, hidden_size), output of LSTM
                   hidden0 - torch.Tensor, initial hidden_state
                   cell0 - torch.Tensor, initial cell_state

            output:
                   lambdas - torch.Tensor, size = (batch size, sequence length, num_classes)
        """
        bs, seq_len, _ = out.shape
